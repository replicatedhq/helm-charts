...
With the above configuration, an annotations containing a hash of the configmap and secret will be added to the pod template. When the configmap or secret changes, the pod will be restarted.
...
This feature is not yet supported when a configmap or secret is referenced in `env` as today `env` only accepts a simple key/value pair format.
